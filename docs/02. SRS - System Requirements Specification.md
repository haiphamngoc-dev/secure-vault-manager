...
| **ChaCha20-Poly1305** | ChaCha20 stream cipher combined with Poly1305 MAC (Mã hóa đối xứng kết hợp xác thực mạnh) |
...
- **FR-WASM-02 (Mã hóa dữ liệu):**
  - Sử dụng thuật toán **ChaCha20-Poly1305** để mã hóa kho dữ liệu JSON.
  - Mỗi lần mã hóa phải sử dụng một Nonce ngẫu nhiên dài 96-bit (12 bytes).
  - Đầu ra mã hóa có cấu trúc: `Salt (16 bytes) || Nonce (12 bytes) || Ciphertext`.
