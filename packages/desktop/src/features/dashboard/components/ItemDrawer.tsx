...
export const ItemDrawer = React.memo(function ItemDrawer({
  ...
  useEffect(() => {
    ...
    setExtraSections([]);
    setWebsites(getInitialWebsites(item));
    setNotes(item.notes || "");
    setTags(item.tags || []);
    setTagInput("");
    setIcon(item.icon);
  ...
  }, [item, getInitialFormFields, getInitialWebsites]);
  ...
  const handleWebsiteBehaviorChange = (index: number, behavior: string) => {
    setWebsites((prev) => {
      const copy = [...prev];
      copy[index] = { ...copy[index], autofillBehavior: behavior };
      return copy;
    });
  };
  ...
  const handleRemoveWebsite = (index: number) => {
    setWebsites((prev) => prev.filter((_, idx) => idx !== index));
  };
                            ...
                            style={{
                              borderBottom:
                                idx < item.urls!.length - 1
                                  ...
                                  : "none",
                              paddingBottom:
                                idx < item.urls!.length - 1 ? "8px" : "0",
                            }}
                          >
                            <Group justify="space-between" align="center">
                              <Text
                                className={classes.fieldValueLink}
                                onClick={() => handleOpenWebsite(web.url)}
                            ...
                            <Group gap="xs" mt="xs">
                              ...
                              >
                                {web.autofillBehavior === "never"
                                  ? t("autofillNeverBadge", "Never fill")
                                  : web.autofillBehavior === "exact"
                                    ? t("autofillExactBadge", "Exact host")
                                    : t(
                                        "autofillAnywhereBadge",
                                        "Fill anywhere"
                                      )}
                              </Badge>
                            </Group>
                      ...
                      <Box>
                        ...
                        <Group gap="xs" mt="xs">
                          ...
                          <Badge
                            ...
                            variant="light"
                            color={
                              item.autofillBehavior === "never"
                                ? "red"
                                : item.autofillBehavior === "exact"
                                  ? "orange"
                                  : "blue"
                            }
                          >
                            {item.autofillBehavior === "never"
                              ...
                              : item.autofillBehavior === "exact"
                                ? t("autofillExactBadge", "Exact host")
                                : t("autofillAnywhereBadge", "Fill anywhere")}
                          </Badge>
                        </Group>
                      </Box>
                    ) : null}
